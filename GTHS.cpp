...
struct PlayerInfo {
	...
	bool boughtDAV = false; // chaos curse wand
	bool boughtRFS = false; // rayman
	bool boughtFYE = false; // chaos drag
	bool boughtAAC = false; // chaos drag
	
	// SHOP ITEMS
	// ROLES
	bool boughtMOD = false; // mod
	bool boughtVIP = false; // vip
...
void banlogin(ENetPeer* peer) {
	string text = "action|log\nmsg|`4Sorry, this account (`5" + ((PlayerInfo*)(peer->data))->rawName + "`4) has been suspended. Dm Coldtopia#7567 to get more info!.\n";
...
int PlayerDB::playerLogin(ENetPeer* peer, string username, string password) {
	...
	if (ifs.is_open()) {
		...
		for (currentPeer = server->peers;
			...
			else if (((PlayerInfo*)(peer->data))->gameversion != "2.995" && ((PlayerInfo*)(peer->data))->gameversion != "2.994") {

				string text = "action|log\nmsg|`4UPDATE REQUIRED`o : The `$V2.995 `oupdate is now avallable for your device. Go get it! You'll need that before you can login into private server.\n";
				string text3 = "action|logon_fail\n";
...
int PlayerDB::playerRegister(ENetPeer* peer, string username, string password, string passwordverify, string discord, string pin) {
	...
	if (username.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") != string::npos) {
		...
		j["boughtFYE"] = ((PlayerInfo*)(peer->data))->boughtFYE;
		j["boughtAAC"] = ((PlayerInfo*)(peer->data))->boughtAAC;
		j["boughtMOD"] = ((PlayerInfo*)(peer->data))->boughtMOD;
		j["boughtVIP"] = ((PlayerInfo*)(peer->data))->boughtVIP;
		j["boughtMEM"] = ((PlayerInfo*)(peer->data))->boughtMEM;
...
void loadConfig()
{
	cout << "Server made by Tox1c and Amelia! Subscribe Tox1c on youtube! >> Channel : Tox1c" << endl;
...
public:
	void operator()(ENetPeer* peer, string playerCalled) {
		if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 400 || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 500)
		{
...
bool isSpecial(string username, string password) {
	for (int i = 0; i < admins.size(); i++) {
		Admin admin = admins[i];
		if (admin.username == username && admin.password == password && admin.level == 32) {
			return true;
		}
	}
	return false;
...
bool isCustomSB(string username, string password) {
	for (int i = 0; i < admins.size(); i++) {
		Admin admin = admins[i];
		if (admin.username == username && admin.password == password && admin.level > 50) {
			return true;
		}
	}
	return false;
...
void sendWorld(ENetPeer* peer, WorldInfo* worldInfo)
{
	...
	if (worldproperlock == false)
	{
		...
		if (((PlayerInfo*)(peer->data))->haveGrowId) {
			...
			int bac, han, fac, hai, fee, pan, nec, shi, mas, anc, ban, lgk, lgw, lgb, lgd, lkw, dav, rfs, fye, aac, mod, vip, mem;
			bac = j["ClothBack"];
			han = j["ClothHand"];
			fac = j["ClothFace"];
			hai = j["ClothHair"];
...
void sendPlayerLeave(ENetPeer* peer, PlayerInfo* player)
{
	...
	count++;
	GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnRemove"), "netID|" + std::to_string(player->netID) + "\n")); // ((PlayerInfo*)(server->peers[i].data))->tankIDName
	GamePacket p4 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Where would you like to go? ( `w" + std::to_string(count) + " `oonline )"));
...
void sendTileUpdate(int x, int y, int tile, int causedBy, ENetPeer* peer)
{
	...
	if (!isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass))
	{
		...
		if (tile == 6 || tile == 8 || tile == 3760 || tile == 1790)
		{
			...
			/*if (world->name != "ADMIN") //todo1 {
				if (world->owner != "") {
					...
					else if (world->isPublic)
					{
						...
						{
							string ownername = world->Displayowner;
							GamePacket p2 = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), ((PlayerInfo*)(peer->data))->netID), "`0" + ownername + "'s `$World Lock`0.(`4No access`0)"), 0));
	...
	if (tile == 0 || tile == 18) {
		...
		if ((duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items[x + (y*world->width)].breakTime >= 4000)
		{
				...
				if (world->items[x + (y*world->width)].foreground != 0)
				{
					...
					if (world->items[x + (y*world->width)].foreground == 242)
					{
						...
						if (((PlayerInfo*)(peer->data))->isNicked == true)
						{
							...
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 && ((PlayerInfo*)(peer->data))->rawName == "coldtopia")
							{
								
								((PlayerInfo*)(peer->data))->country = "../rtsoft_logo";
							}
							if (((PlayerInfo*)(peer->data))->boughtVIP == true)
							{
								
								((PlayerInfo*)(peer->data))->country = "../atomic_button";
							}
							if (((PlayerInfo*)(peer->data))->boughtMOD == true)
							{
								
								((PlayerInfo*)(peer->data))->country = "../token_icon_overlay";
							}
			...
			if (world->items[x + (y * world->width)].foreground == 6026)
				if (((PlayerInfo*)(peer->data))->boughtFYE == false)
				{
					GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
					ENetPacket* packet = enet_packet_create(p.data,
						p.len,
						ENET_PACKET_FLAG_RELIABLE);
					enet_peer_send(peer, 0, packet);
				...
				else
				{
					...
					if (world->items[x + (y * world->width)].foreground == 2952)
			...
			if (tile == 242) {
					...
					++currentPeer)
				{
					...
					if (isHere(peer, currentPeer)) {
						...
						if (((PlayerInfo*)(peer->data))->rawName == "coldtopia" && getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999)
						{
							world->pIsPlay = true;
						}
						else if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 && ((PlayerInfo*)(peer->data))->rawName != "coldtopia") {
							world->pIsDev = true;
						}
						...
						if (((PlayerInfo*)(peer->data))->rawName == "coldtopia" && getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999)
						{
							...
							enet_peer_send(currentPeer, 0, packet);
							delete p.data;
							GamePacket p2 = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), ((PlayerInfo*)(peer->data))->netID), "`5[`w" + world->name + " `whas been World Locked by `w" + ((PlayerInfo*)(peer->data))->displayName + "`5]"), 0));
							ENetPacket* packet2 = enet_packet_create(p2.data,
								p2.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(currentPeer, 0, packet2);
						...
						else
						{
							...
							{
								if (isMod(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 0)
								{
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`3[`w" + world->name + " `ohas been World Locked by `4" + ((PlayerInfo*)(peer->data))->displayName + "`3]"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
...
void sendPlayerEnter(ENetPeer* peer, PlayerInfo* player)
{
	...
	GamePacket p  = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), ((PlayerInfo*)(peer->data))->netID), "`5<`w" + player->displayName + "`` `5entered, `w" + std::to_string(getPlayersCountInWorld(player->currentWorld)) + "`` `5others here>```w"), 0));
	for (currentPeer = server->peers;
		currentPeer < &server->peers[server->peerCount];
		++currentPeer)
	{
...
void sendPlayerWBan(ENetPeer* peer, string from, string to)
{
		...
		++currentPeerp)
	{
		...
		else
		{
			...
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"),"`o" + name + " `4world bans " + "`o" + kickname + " `ofrom `w" + ((PlayerInfo*)(peer->data))->currentWorld + "!"));
...
void sendChatMessage(ENetPeer* peer, int netID, string message)
{
	...
	if (!((PlayerInfo*)(peer->data))->haveGrowId) {
		...
		if (((PlayerInfo*)(peer->data))->rawName == "styx") 
		{
			ENetPeer* currentPeer;
			string name = "";
			for (currentPeer = server->peers;
		...
		if (((PlayerInfo*)(peer->data))->rawName == "wolf")
		{
			ENetPeer* currentPeer;
			string name = "";
			for (currentPeer = server->peers;
		...
		if (((PlayerInfo*)(peer->data))->rawName == "legal")
		{
			ENetPeer* currentPeer;
			string name = "";
			for (currentPeer = server->peers;
		...
		if (((PlayerInfo*)(peer->data))->rawName == "focused")
		{
			ENetPeer* currentPeer;
			string name = "";
			for (currentPeer = server->peers;
		...
		if (((PlayerInfo*)(peer->data))->rawName == "joergen")
		{
			ENetPeer* currentPeer;
			string name = "";
			for (currentPeer = server->peers;
		...
		if (((PlayerInfo*)(peer->data))->boughtMEM == false) {
			GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequestad"), "set_default_color|`o\n\nadd_label_with_icon|big|`2Rules For Staff``|left|5956|\n\nadd_spacer|small|\nadd_label_with_icon|small|Do not abuse your power|left|1432|\nadd_label_with_icon|small|Do not begging for a higher role|left|1432|\nadd_label_with_icon|small|Do not fban/mute people without reason!|left|1432|\nadd_label_with_icon|small|If you want to ban/ipban a players,make sure to record / screenshot and send it to ipadtopia#7567.|left|1432|\nadd_label_with_icon|small|Do not spam all customsb or staff sb.|left|1432|\nadd_label_with_icon|small|\nadd_label_with_icon|small|Ipadtopia - Fliqx - Secret|left|1432|\nadd_spacer|small| |left|6746|\nadd_quick_exit|\nadd_button|chc0|Close|noflags|0|0|\nnend_dialog|gazette||OK|"));
			ENetPacket* packet = enet_packet_create(p.data,
				p.len,
				ENET_PACKET_FLAG_RELIABLE);
			enet_peer_send(peer, 0, packet);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	*p = 5;*/
	...
	addAdmin("tuxxy", "tuxxy1234", 400);
	addAdmin("tuxxy", "tuxxy1234", 166);
	addAdmin("tuxxy", "tuxxy1234", 400);
	addAdmin("tuxxy", "tuxxy1234", 300);
	addAdmin("coldtopia", "ihatemyself123", 999);
	addAdmin("coldtopia", "ihatemyself123", 333);
	addAdmin("coldtopia", "ihatemyself123", 666);
	addAdmin("coldtopia", "ihatemyself123", 1337);
	addAdmin("massy", "massy123456", 4);
	addAdmin("massy", "massy123456", 3);
	addAdmin("massy", "massy123456", 666);
	addAdmin("massy", "massy123456", 333);
	addAdmin("legal", " bagus1234", 666);
	addAdmin("legal", "bagus1234", 333);
	addAdmin("axeltopia", "azazerty123", 333);
	// load items.dat
	{
	...
	while (true)
		while (enet_host_service(server, &event, 1000) > 0)
		{
			...
			case ENET_EVENT_TYPE_RECEIVE:
			{
				...
				if (((PlayerInfo*)(event.peer->data))->haveGuestId == true)
				{
					if (!((PlayerInfo*)(event.peer->data))->haveGrowId)
					{
						...
						((PlayerInfo*)(event.peer->data))->displayName = "`o(`2Guest`o)" + PlayerDB::fixColors(((PlayerInfo*)(event.peer->data))->requestedName.substr(0, ((PlayerInfo*)(event.peer->data))->requestedName.length() > 15 ? 15 : ((PlayerInfo*)(event.peer->data))->requestedName.length()) + "_" + std::to_string(event.peer->address.host));
				...
				if (((PlayerInfo*)(event.peer->data))->wrongpass == true)
				{
					...
					{
						string text = "action|log\nmsg|`4Unable to log on:``That `0GrowID ``Doesn't seem valid, or the password is wrong. if you dont have one,click `0Cancel,``un-check `0'i have a GrowID'``,then click `0Connect.``\n";
						string text3 = "action|logon_fail\n";
						string dc = "https://invite.gg/gtit";
						string url = "action|set_url\nurl|" + dc + "\nlabel|Join discord\n";
					...
					if (exist == true)
					{
						if (peer->address.host == currentPeer->address.host)
						{
							...
							else
							{
								...
								string text = "action|log\nmsg|`4Sorry, this device or location has been suspended.If you have question dm ColdTopia#7567 on discord!";
								string text3 = "action|logon_fail\n";
								string dc = "https://invite.gg/gtit";
								string url = "action|set_url\nurl|" + dc + "\nlabel|Join discord\n";
				...
				case 2:
				{
								...
								if (((PlayerInfo*)(currentPeer->data))->netID == ((PlayerInfo*)(peer->data))->wrenchsession) {
									...
									if (rawnamofwrench != rawnam)
									{
										...
										if (rawnamofwrench != "")
										{
											...
											if (world->owner == ((PlayerInfo*)(peer->data))->rawName && ((PlayerInfo*)(peer->data))->haveGrowId || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) > 666)
											{
												if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) > 666)
												{
													GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\nadd_label_with_icon|big|`w" + name + "|left|18|\nadd_spacer|small|\nadd_label|small|`6Gems:`w " + contentch2x + "|left|4|\n\nadd_button|starttrd|Trade|noflags|0|0|\nadd_button|punishview|`1Punish/View|noflags|0|0|\nadd_button|kick|`4Kick|0|0|\nadd_button|pull|`5Pull|0|0|\nadd_button|wban|`4World Ban|0|0|\nadd_button|addfriendrnbutton|`0Add as friend|noflags|0|0|\nadd_button|yesmod|`wPromote to `1VIP|noflags|0|0|\nadd_spacer|small|\nadd_button||Continue|0|0|\nadd_quick_exit"));
													ENetPacket * packet = enet_packet_create(p.data,
														p.len,
														ENET_PACKET_FLAG_RELIABLE);
													enet_peer_send(peer, 0, packet);
												...
												{
													GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\nadd_label_with_icon|big|`w" + name + "|left|18|\nadd_spacer|small|\nadd_label|small|`6Gems:`w " + contentch2x + "|left|4|\n\nadd_button|starttrd|Trade|noflags|0|0|\nadd_button|punishview|`1Punish/View|noflags|0|0|\nadd_button|kick|`4Kick|0|0|\nadd_button|pull|`5Pull|0|0|\nadd_button|wban|`4World Ban|noflags||0|0|\nadd_button|addfriendrnbutton|`0Add as friend|noflags|0|0|\nadd_spacer|small|\nadd_button||Continue|0|0|\nadd_quick_exit"));
													ENetPacket* packet = enet_packet_create(p.data,
														p.len,
														ENET_PACKET_FLAG_RELIABLE);
													enet_peer_send(peer, 0, packet);
									...
									else
									{
										...
										else
										{
											if (((PlayerInfo*)(peer->data))->haveGrowId == true)
											{
												GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\nadd_label_with_icon|big|`0" + name + "|left|18|\nadd_spacer|small|\nadd_label|small|`oYou have:`2 " + contentch2 + " `oGems|left|4|\nadd_button|enableaap|`5Your Current role : `wPlayer``|noflags|0|0|\n\nadd_button|chc0|Close|noflags|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
												ENetPacket * packet = enet_packet_create(p.data,
													p.len,
													ENET_PACKET_FLAG_RELIABLE);
												enet_peer_send(peer, 0, packet);
											...
											if (((PlayerInfo*)(peer->data))->haveGrowId == true && ((PlayerInfo*)(peer->data))->boughtMOD == true)
											{
												GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\nadd_label_with_icon|big|`w" + name + "|left|278|\nadd_spacer|small|\nadd_label|small|`oYour current `6Gems:`w " + contentch2 + "|left|4|\nadd_label|small|Your current `4Role:`1VIP|left|4|\nadd_button||Continue|0|0|\nadd_quick_exit"));
												ENetPacket* packet = enet_packet_create(p.data,
													p.len,
													ENET_PACKET_FLAG_RELIABLE);
												enet_peer_send(peer, 0, packet);
											...
											if (((PlayerInfo*)(peer->data))->haveGrowId == true && ((PlayerInfo*)(peer->data))->boughtVIP == true)
											{
												GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\nadd_label_with_icon|big|`w" + name + "|left|278|\nadd_spacer|small|\nadd_label|small|`oYour current `6Gems:`w " + contentch2 + "|left|4|\nadd_label|small|Your current `4Role:`^MODERATOR|left|4|\nadd_button||Continue|0|0|\nadd_quick_exit"));
												ENetPacket* packet = enet_packet_create(p.data,
													p.len,
													ENET_PACKET_FLAG_RELIABLE);
												enet_peer_send(peer, 0, packet);
					...
					if (cch.find("action|growid") == 0)
					{
						...
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wGet a GrowID``|left|206|\n\nadd_spacer|small|\nadd_textbox|`$Welcome To GrowtopiaTC! You can make your Growid here.|\nadd_spacer|small|\nadd_textbox|Please select a kindly name in ur account,because if you are using a sexual name or something your account can be `4Deleted`0!|\nadd_text_input|username|GrowID :||30|\nadd_text_input|password|Password :||100|\nadd_text_input|passwordverify|Password Verify :||100|\nadd_text_input|discord|Discord :||100|\nadd_text_input|pin|Pin :||4|\nend_dialog|register|Cancel|`2Create my account!|\n"));
						ENetPacket* packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
					...
					if (cch.find("action|store") == 0) // to do and sound (for example breaking when no access is to do) and accessing on wls.
					{
						if (((PlayerInfo*)(peer->data))->haveGrowId == true)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label|small|`wWelcome to our Store!|left|5016|\n\nadd_spacer|small|\nadd_label|store|`rPlease choose a button that you want to buy.|left|1488|\nadd_spacer|small| |left|212|\nadd_button|store|`wPurchase `2in-game `5Assets |noflags|1232|nadd_spacer|small|\nadd_button|items|`wPurchase `6in-game `eItems|noflags|3233|\nadd_button|close|Close|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
					...
					if (cch.find("action|dialog_return") == 0)
					{
							...
							vector<string> infoDat = explode("|", to);
							{
								...
								if (dialog_name == "captcha") {
									if (btn == pData->buttonID) {
										...
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`eGrowtopiaTC``|left|2252|\n\nadd_spacer|small|\nadd_label_with_icon|small|\nadd_button|crash|`2Press This button if you are `4crashing`w!|noflags|0|0|\n\nadd_image_button|banner|interface/large/anni_sta.rttex|noflags|||\nadd_textbox|`oWelcome to GrowtopiaTC!|left|6746|\nadd_spacer|small| |left|6746|\nadd_label_with_icon|small|\nadd_textbox|Dear GrowtopiaTC Players,|left|6746|\nadd_label_with_icon|small| |left|6746|\nadd_label_with_icon|small|\nadd_textbox| GrowtopiaIF has been closed.This is the new server of GrowtopiaIF called GrowtopiaTC!|left|6746|\nadd_label_with_icon|small| |left|6746|\nadd_label_with_icon|small|\nadd_url_button||`0Join Project IT Discord server``|NOFLAGS | https://invite.gg/projectit|Open link?|0|0| |left|6746|\nadd_spacer|small| |left|6746|\nadd_label_with_icon|small| |left|6746|\nadd_label_with_icon|small|`4WARNING:`` `5Worlds (and accounts)`` might be deleted at any time if database issues appear (once per day or week).|left|1432|\nadd_label_with_icon|small| |left|6746|\nadd_label_with_icon|small|`4WARNING:`` `5Accounts`` are in beta, bugs may appear and they will be probably deleted often, because of new accounts updates, which will cause database incompatibility.|left|1432|\nadd_label_with_icon|small| |left|6746|\nadd_label_with_icon|small|\nadd_url_button||`4Subscribe `eColdTopia `0on You`4Tube!``|NOFLAGS|https://www.youtube.com/c/ipadtopia|open link?|0|0|\nadd_url_button||`4Subscribe `1Tyzies `0on You`4Tube|NOFLAGS|https://www.youtube.com/channel/UCRrW2HRl1cDZqzhxFZlInlg|open link?|0|0|\nadd_quick_exit|\nadd_button|chc0|Close|noflags|0|0|\nnend_dialog|gazette||OK|"));
										ENetPacket * packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(peer, 0, packet);
						...
						if (btn == "vyteserver")
						{
							GamePacket p3 = packetEnd(appendInt(appendInt(appendString(appendString(createPacket(), "OnRedirectServer"), "35.222.84.253 growtopia1.com"), 17353), 1));
							...
							GamePacket pto = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`9You will now warp to GTIO server when connect! `w(to go back restart gt)"));
							//memcpy(p2.data + 8, &(((PlayerInfo*)(currentPeer->data))->netID), 4);
							ENetPacket* packetto = enet_packet_create(pto.data,
								pto.len,
								ENET_PACKET_FLAG_RELIABLE);
						...
						if (btn == "lvl") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wPurchase `3Levels``|left|1488|\n\nadd_spacer|small|\nadd_spacer|small| |left|1426|\nadd_label|small|Price: `110/`6World lock|left|1432|\nadd_label|small|`oStock: `w[`4~`w]|left|1432|\nadd_label|small|Duration: `w[`4~`0] |left|1432|\nadd_label|small|`6Rules:|left|1432|\nadd_label|small|`61.`2Trying to sell ur account will result in a `4IPBAN`0.|left|3132|\nadd_spacer|small| |left|212|\nadd_label|small|`eHow to buy:|left|212|\nadd_label|small|`rPlace the diamond locks on the donation box in GTIFDEPOSIT in Normal Growtopia,Write ur GttcID.Also dont forgot to add `3#Levels.|left|212|\nadd_button|back|back|noflags|0|0|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "legenddragon") {
							...
							{
								if (((PlayerInfo*)(event.peer->data))->boughtLGD == true)
						...
						if (btn == "legendwhip") {
							...
							{
								if (((PlayerInfo*)(event.peer->data))->boughtFYE == true)
						...
						if (btn == "legendkatana") {
							...
							{
								if (((PlayerInfo*)(event.peer->data))->boughtLGK == true)
						...
						if (btn == "legendwing") {
							...
							{
								if (((PlayerInfo*)(event.peer->data))->boughtLGW == true)
						...
						if (btn == "legendbot") {
							...
							{
								if (((PlayerInfo*)(event.peer->data))->boughtLGB == true)
						...
						if (isFindDialog && btn.substr(0, 4) == "tool") {
							...
							if (proitem == 1874 || proitem == 1876 || proitem == 1986 || proitem == 2970 || proitem == 1780 || proitem == 1782 || proitem == 1784 || proitem == 7734 || proitem == 5026)
							{
								...
								GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`2Item `w " + id + " `ohas been `2added `oto your inventory"));
								ENetPacket* packet2 = enet_packet_create(p2.data,
									p2.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet2);
...
#pragma warning (disable : 4996)
						...
						if (btn == "dav")
						{
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wAre you sure to buy `6Davinci wings `w for `523.000 `4Gems`w?``|left|6128|\nadd_button|yesdav|Yes!|0|0|\nadd_button|no|Back|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "rfs")
						{
							if (((PlayerInfo*)(peer->data))->boughtRFS == false)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wAre you sure to buy `bDigger spade `w for `5100.000 `4Gems`w?``|left|6128|\nadd_label|small|`2This item contains: `w You will get 20-30 gems for breaking gems + 2 hit blocks\nadd_button|yesrfs|Yes!|0|0|\nadd_button|no|Back|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "fye")
						{
							if (((PlayerInfo*)(peer->data))->boughtFYE == false)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wAre you sure to buy `6Whip Of truth for `5500.000 Gems!|left|6128|\nadd_label|small|`2This item contains: `w/ssb|\nadd_button|yesfye|Yes!|0|0|\nadd_button|no|Back|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "aac")
						{
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wAre you sure to buy `6Ancestral for `5300.000 `6Gems?|left|6128|\nadd_label|small|`2This item contains: /bluename|\nadd_button|yesanc|Yes!|0|0|\nadd_button|no|Back|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "mem")
						{
							if (((PlayerInfo*)(peer->data))->boughtMEM == false)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wAre you sure to buy `6CHAT for `51000 `6Gems?|left|6128|\nadd_label|small|`2This item contains: Chat in public|\nadd_button|yesmem|Yes!|0|0|\nadd_button|no|Back|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wYou Already Bought this item.``|left|6126|\nadd_button|no|Back|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							if (((PlayerInfo*)(peer->data))->boughtMOD == true)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wYou Cant buy this item.``|left|6126|\nadd_button|no|Back|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "yesdav") {
							...
							if (b > 22999)
							{
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r`2TRANSACTION SUCCESS! - You now bought Davinci wings!"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "yesrfs") {
							...
							if (b > 99999)
							{
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r`2TRANSACTION SUCCESS! - `2Congratulations! `wYou bought `bDigger Spade`w!"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "yesfye") {
							...
							if (b > 499999)
							{
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r`2TRANSACTION SUCCESS! - You now bought Whip of Truth!"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "yesanc") {
							...
							if (b > 299999)
							{
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r`2TRANSACTION SUCCESS! - `wCongratulations! You bought ancestral."));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "yesmem") {
							...
							if (b > 999)
							{
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r`2TRANSACTION SUCCESS! - `wCongratulations! You bought Chat! now you can speak in the public."));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r`4TRANSACTION FAILED! - `wYou need to have enough gems to buy `6CHAT`o!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (isRegisterDialog) {
							...
							if (regState == 1) {
								((PlayerInfo*)(peer->data))->haveGrowId = false;
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`eYour account has successfully created!``"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
								delete p.data;
								GamePacket p3 = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big| `wGrowid GET!``|left|242|\nadd_label|small|`oA `oGrowid `oWith the log on of `w" + username + " `ohas been created.Write it and your password down as they will be required log on from now on!|left|323||\nadd_spacer|small|\nadd_quick_exit|\nadd_button|chc0|Close|noflags|0|0|\nnend_dialog|gazette||OK|``"));
								ENetPacket* packet3 = enet_packet_create(p3.data,
									p3.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet3);
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Oops! `9Looks like players data have been resetted, `wmake sure to use your `bPIN `wthat you used when creating your account. `2For help, message `wColdtopia#&567 on Discord!`w``"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
					...
					if (cch.find("text|") != std::string::npos) {
						...
						{
							if (((PlayerInfo*)(peer->data))->boughtMOD == false)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oYou need to be `1VIP `obefore u can do /ghost!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
								...
								if (((PlayerInfo*)(peer->data))->boughtVIP == true) {
									...
									else
									{
										...
										if (((PlayerInfo*)(peer->data))->boughtVIP == true) {
											...
											if (((PlayerInfo*)(currentPeer->data))->rawName == "coldtopia")
											{
							...
							if (x == "")
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`^Moderators online: (All are hidden)."));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (x == "")
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`1Vip online: (All are hidden)."));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`1Vip online: " + x));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
					...
					delete p.data;
						...
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`eGrowtopiaTC >>`5Players online: `w" + online));
						ENetPacket* packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
							...
							++currentPeer)
						{
						    ...
						    if (((PlayerInfo*)(peer->data))->boughtVIP == false)
						    {
							   GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`wYou need to be `^Moderator `wto do that!"));
							   ENetPacket* packet = enet_packet_create(p.data,
								  p.len,
								  ENET_PACKET_FLAG_RELIABLE);
							   enet_peer_send(peer, 0, packet);
						...
						{
							if (name == "CON") {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oHuh? Why you want warp to con?"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
							...
							if (name == "NUL") {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oHuh? Why you want warp to nul?"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
						...
						{
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) > 400) {
						...
						if (((PlayerInfo*)(peer->data))->boughtMOD == false)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You need to be `1VIP `oBefore you can do that!"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (((PlayerInfo*)(peer->data))->boughtMOD == false) {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4ERROR`W>>`oYou need to be `1VIP `oto do that!"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						{
							cout << "FBAN from " << ((PlayerInfo*)(peer->data))->rawName << " in world " << ((PlayerInfo*)(peer->data))->currentWorld << "with IP " << peer->address.host << " Fake Banned user:" << str.substr(6, cch.length() - 6 - 1) << endl;
							GamePacket ban = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `$The Ancients Ones `ohave `4banned `0" + str.substr(6, cch.length() - 6 - 1) + " `#** `o(`4/rules`o to see the rules!)"));
								...
								++currentPeer)
							{
								...
								if (((PlayerInfo*)(currentPeer->data))->rawName == name) {
									GamePacket p = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/atomic_button.rttex"), "`wWarning from `4System: `wYou've been `4BANNED `wfrom Growtopia for 729 days"), "audio/hub_open.wav"), 0));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(currentPeer, 0, packet);

									GamePacket p1 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`$Warning from `4System`$: You've been `4BANNED `$from Growtopia for 729 days"));
							...
							else
							{
									...
									++currentPeer)
								{
									...
									if (((PlayerInfo*)(currentPeer->data))->rawName == PlayerDB::getProperName(str.substr(5, cch.length() - 5 - 1))) {
										...
										GamePacket ps2 = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/atomic_button.rttex"), "`0Warning from `4System`0: You've been `4BANNED `0from GrowtopiaTC for 730 days"), "audio/hub_open.wav"), 0));
										ENetPacket* packet2 = enet_packet_create(ps2.data,
											ps2.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(currentPeer, 0, packet2);
										GamePacket ps3 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oWarning from `4System`o: You've been `4BANNED `ofrom GrowtopiaTC for 730 days"));
										ENetPacket* packet3 = enet_packet_create(ps3.data,
											ps3.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(currentPeer, 0, packet3);
										...
										dupa = ((PlayerInfo*)(currentPeer->data))->rawName;
										if (((PlayerInfo*)(peer->data))->boughtVIP == false) {
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessages"), "`e[MOD LOGS]: `r" + dupa + "`r have used Curse on  `1" + str.substr(6, cch.length() - 6 - 1)));
											ENetPacket* packet = enet_packet_create(p.data,
												p.len,
												ENET_PACKET_FLAG_RELIABLE);
											enet_peer_send(currentPeer, 0, packet);
											...
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`e[MOD LOGS]: `r" + dupa + "`rJust used `4BAN `ron " + str.substr(5, cch.length() - 5 - 1)));
											ENetPacket* packet = enet_packet_create(p.data,
												p.len,
												ENET_PACKET_FLAG_RELIABLE);
											enet_peer_send(currentPeer, 0, packet);
						...
						else
						{
							...
							if (!found) {
						...
						else if (str.substr(0, 10) == "/givemode ") {

								if (!isOwner(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) break;
								string name = str.substr(10, str.length());
								GamePacket p = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/atomic_button.rttex"), "`0You have `2Received `^Moderator`w!"), "audio/hub_open.wav"), 0));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(currentPeer, 0, packet);
								...
								if (((PlayerInfo*)(currentPeer->data))->rawName == name) {
									...
									((PlayerInfo*)(peer->data))->boughtMOD = true;
									...
									j["boughtMOD"] = ((PlayerInfo*)(peer->data))->boughtMOD; //edit
						...
						else if (str.substr(0, 10) == "/givevipe ") {
							{
								if (!isOwner(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) break;
								string name = str.substr(10, str.length());
								GamePacket p = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/atomic_button.rttex"), "`0You have `2Received `1VIP`w!"), "audio/hub_open.wav"), 0));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(currentPeer, 0, packet);
						...
						else if (str.substr(0, 10) == "/givemods ") {
							{
								if (!isOwner(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) break;
								string name = str.substr(10, str.length());
								GamePacket p = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/atomic_button.rttex"), "`0You have `2Received `1VIP`w!"), "audio/hub_open.wav"), 0));
								...
								((PlayerInfo*)(peer->data))->boughtVIP = true;
								...
								j["boughtVIP"] = ((PlayerInfo*)(peer->data))->boughtVIP; //edit
									...
									++currentPeer)
								{
									...
									if (((PlayerInfo*)(currentPeer->data))->rawName == PlayerDB::getProperName(str.substr(7, cch.length() - 7 - 1))) {
										...
										if (((PlayerInfo*)(peer->data))->boughtVIP == false) {
											...
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`e[MOD LOGS]: `r" + dupa + "`r have used Curse on  `1" + str.substr(6, cch.length() - 6 - 1)));
											ENetPacket* packet = enet_packet_create(p.data,
												p.len,
												ENET_PACKET_FLAG_RELIABLE);
											enet_peer_send(currentPeer, 0, packet);
							...
							else
							{
								...
								if (world->nuked) {
										...
										++currentPeer)
									{
										...
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`o>> `4" + world->name + " has been nuked from orbit`w. `oIt's the only way to be sure. Play safe, everybody!"));
										ENetPacket* packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(currentPeer, 0, packet);
										...
										if (((PlayerInfo*)(peer->data))->boughtVIP == false) {
											...
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`e[MOD LOGS]: `r" + dupa + "`rJust nuked world `4" + world->name));
											ENetPacket* packet = enet_packet_create(p.data,
												p.len,
												ENET_PACKET_FLAG_RELIABLE);
											enet_peer_send(currentPeer, 0, packet);
						...
						if (world->owner == ((PlayerInfo*)(peer->data))->rawName && ((PlayerInfo*)(peer->data))->haveGrowId)
						{
							namespace fs = std::experimental::filesystem;
						...
						if (((PlayerInfo*)(peer->data))->boughtAAC == false)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Error! `w>> You need to `4Purchase `9Ancestral `w to do that!"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (str == "/vhelp")
						{
							if (((PlayerInfo*)(peer->data))->boughtMOD == false)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Error! `w>> You need to be `1VIP `w to do that!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`1Vip Command are `0>>  /ghost, /nick (need atleast 5 characters), /warp, /spawn (id), /eff (id), /ducttape, /magic, /server, /breakroulette, /vsb, /invis"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (((PlayerInfo*)(peer->data))->boughtMOD == false)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oYou need to be `1VIP `oBefore you can do /vsb!"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						{
							string name = ((PlayerInfo*)(peer->data))->displayName;
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w** `1VIP-SB`` from `$`6" + name + "`w (in `4HIDDEN!``) ** :`7 " + str.substr(4, cch.length() - 4 - 1)));
							string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
							BYTE* data = new BYTE[5 + text.length()];
							BYTE zero = 0;
							int type = 3;
						...
						else if (str.substr(0, 5) == "/vch ") {
							using namespace std::chrono;
							if (((PlayerInfo*)(peer->data))->boughtMOD == false)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oYou need to be `1VIP `oBefore you can do /v!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							else
							{
								if (((PlayerInfo*)(peer->data))->boughtMOD == false)
								{
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessagesz"), "`oYou need to be `1VIP `oBefore you can do /v!"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(peer, 0, packet);
									...
									string name = ((PlayerInfo*)(peer->data))->displayName;
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`e[VIP-CHAT] from `6" + name + "`1" + str.substr(4, cch.length() - 4 - 1)));
									string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
									BYTE* data = new BYTE[5 + text.length()];
									BYTE zero = 0;
									int type = 3;
						...
						else if (str.substr(0, 5) == "/mch ") {
							using namespace std::chrono;
							if (((PlayerInfo*)(peer->data))->boughtVIP == false)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oYou need to be `^Moderator `oBefore you can do /v!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							else
							{
								if (((PlayerInfo*)(peer->data))->boughtVIP == false)
								{
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessages"), "`oYou need to be `1VIP `oBefore you can do /v!"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(peer, 0, packet);
									...
									string name = ((PlayerInfo*)(peer->data))->displayName;
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`^[MOD-CHAT] from `6" + name + "`1" + str.substr(4, cch.length() - 4 - 1)));
									string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
									BYTE* data = new BYTE[5 + text.length()];
									BYTE zero = 0;
									int type = 3;
							...
							if (((PlayerInfo*)(peer->data))->boughtVIP == false)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oYou need to be `^Moderator `oBefore you can do /vsb!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								string name = ((PlayerInfo*)(peer->data))->displayName;
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w** `bMod-SB`w from `$`6" + name + "`w (in `4HIDDEN!`w) ** :`^" + str.substr(4, cch.length() - 4 - 1)));
								string text = "action|play_sfx\nfile|audio/achievement.wav\ndelayMS|0\n";
								BYTE* data = new BYTE[5 + text.length()];
								BYTE zero = 0;
								int type = 3;
						...
						if (((PlayerInfo*)(peer->data))->boughtFYE == false)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4ERROR `w>> You will need to buy `6Whip of truth `wbefore u can do /ssb!"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						{
							string name = ((PlayerInfo*)(peer->data))->displayName;
							GamePacket p3 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You received `1S`2P`3E`4C`5I`6A`9L`0-`2SB From `9" + name + ""));
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w** `1S`2P`3E`4C`5I`6A`9L`0-`2SB`` from `$`2" + name + "`w (in `$" + ((PlayerInfo*)(peer->data))->currentWorld + "``) ** :`^" + str.substr(4, cch.length() - 4 - 1)));
						...
						if (!isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) break;
						string name = ((PlayerInfo*)(peer->data))->displayName;
						GamePacket p3 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You received `9Owner-SB From `9" + name + ""));
						...
						if (str == "/store") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wPurchase `eingame Assets``|left|1430|\n\nadd_spacer|small|\nadd_button|mods|`wPurchase `^Moderator|noflags|4758|\nadd_button|vip|`wPurchase `1VIP|noflags|3798|\nadd_button|gems|`wPurchase `6Gems|left|112|\nadd_button|lvl|`wPurchase `3Level|noflags|1988|\nadd_quick_exit|\nadd_button|chc0|Close|noflags|0|0|\nnend_dialog|gazette||OK|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`1Vip Command are `0>> /help, /dance, /furious, /love, /cry, /mad, /sleep, /yes, /no, /troll, /cheer, /fp, /omg, /fa, /rolleyes, /dab, /idk, /shrug, /love, /mods, /pull, /rgo, /r, /rules, /news, /report <name>, /mod, /unmod, /inventory, /item id, /team id, /color number, /who, /count, /sb message, /alt, /radio, /find, /pay (user) (amount), /cleaninv, /unequip, /msg, /block, /uba, /jsb, /ghost, /nick (need atleast 5 characters), /warp, /spawn (id), /eff (id), /ducttape, /magic, /server, /breakroulette, /vsb"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`^Moderator Command are `o>> /help, /dance, /furious, /love, /cry, /mad, /sleep, /yes, /no, /troll, /cheer, /fp, /omg, /fa, /rolleyes, /dab, /idk, /shrug, /love, /mods, /pull, /rgo, /r, /rules, /news, /report <name>, /mod, /unmod, /inventory, /item id, /team id, /color number, /who, /count, /sb message, /alt, /radio, /find, /pay (user) (amount), /cleaninv, /unequip, /msg, /block, /effect, /uba, /jsb, /nick, /ban, /deviceban (ipID), /remove (ipID), /eff (id), /ducttape, /nuke, /summon (name), /warp (world), /warpto (player), /online, /offlineinfo (name), /ghost, /invis, /curse (name), /uncurse (name), /unban (name), /spawn (id), /asb, /magic, /boot, /ipcheck <name>, /nicked, /server, /banworld, /breakroulette, /vsb"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (str == "/mhelp")
						{
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`^Moderator Command are `o>> /nick, /ban, /deviceban (ipID), /remove (ipID), /eff (id), /ducttape, /nuke, /summon (name), /warp (world), /warpto (player), /online, /offlineinfo (name), /ghost, /invis, /curse (name), /uncurse (name), /unban (name), /spawn (id), /asb, /magic, /boot, /ipcheck <name>, /nicked, /server, /banworld, /breakroulette, /vsb"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (((PlayerInfo*)(peer->data))->currentWorld != "EXIT")
						{
							if (((PlayerInfo*)(peer->data))->boughtMOD == false)
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`wYou need to be `1VIP `wBefore u can do that!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							++currentPeer)
						{
							...
							if (((PlayerInfo*)(peer->data))->boughtVIP == false)
						    {
							    GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oYou need to be `^Moderator `oBefore you can do this"));
							    ENetPacket* packet = enet_packet_create(p.data,
								   p.len,
								   ENET_PACKET_FLAG_RELIABLE);
							    enet_peer_send(peer, 0, packet);
							...
							else
							{
								...
								if (playerCalled == "ipadtopia" || playerCalled == "random" || playerCalled == "raiterjaki" || playerCalled == "esc") {
								...
								else
								{
										...
										++currentPeer)
									{
										...
										if (((PlayerInfo*)(currentPeer->data))->rawName == playerCalled)
										{
											...
											if (getAdminLevel(((PlayerInfo*)(currentPeer->data))->rawName, ((PlayerInfo*)(currentPeer->data))->tankIDPass) == 999 && ((PlayerInfo*)(peer->data))->rawName != "ipadtopia")
											{
						...
						if (((PlayerInfo*)(peer->data))->boughtVIP == false)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oYou need to be `^Moderator `oBefore you can do /hardban"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (((PlayerInfo*)(peer->data))->boughtMOD == false)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oYou need to be `1VIP `oBefore you can do /nick!"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						else
						{
							...
							if (isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 0 && ((PlayerInfo*)(peer->data))->rawName == "coldtopia")
							{
								...
								++currentPeer)
							{
								...
								if (isHere(peer, currentPeer))
								{
									...
									if (((PlayerInfo*)(peer->data))->boughtVIP == false) {
										...
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`e[MOD LOGS]: `r" + dupa + " just using nick and has renamed to `e" + str.substr(6, cch.length() - 6 - 1)));
										ENetPacket* packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(currentPeer, 0, packet);
						...
						if (!isOwner(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) break;
						sendConsoleMsg(peer, "Welcome back `2Ipadtopia`o!");
						sendPlayerLeave(peer, (PlayerInfo*)(event.peer->data));
						((PlayerInfo*)(peer->data))->currentWorld = "EXIT";
						sendWorldOffers(peer);
						((PlayerInfo*)(peer->data))->cloth_hair = 4820;
						((PlayerInfo*)(peer->data))->cloth_shirt = 3370;
						((PlayerInfo*)(peer->data))->cloth_pants = 344;
						((PlayerInfo*)(peer->data))->cloth_feet = 6780;
						((PlayerInfo*)(peer->data))->cloth_face = 1204;
						((PlayerInfo*)(peer->data))->cloth_hand = 2592;
						((PlayerInfo*)(peer->data))->cloth_back = 5754;
						((PlayerInfo*)(peer->data))->cloth_mask = 4182;
						((PlayerInfo*)(peer->data))->cloth_necklace = 1968;
						((PlayerInfo*)(peer->data))->cloth_ances = 7174;
						sendPlayerLeave(peer, (PlayerInfo*)(event.peer->data));
						((PlayerInfo*)(peer->data))->currentWorld = "EXIT";
						sendWorldOffers(peer);

						string name = ((PlayerInfo*)(peer->data))->displayName;
						((PlayerInfo*)(peer->data))->displayName = "`w[`eIPAD`w]`^ " + name + "";

						GamePacket ps = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`0Welcome back `eIpadtopia!"));
						ENetPacket* packet = enet_packet_create(ps.data,
							ps.len,
							ENET_PACKET_FLAG_RELIABLE);
						...
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`eGrowtopiaTC >> `2There is `2" + std::to_string(count) + " `1people are online."));
						ENetPacket* packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
						...
						{
						if (isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 0 || !isMod(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 0) {
							PlayerMoving data;
							data.packetType = 0x14;
							data.characterState = 0x0; // animation
							data.x = 1000;
						...
						if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999) {
							if (((PlayerInfo*)(peer->data))->rawName == "coldtopia" || ((PlayerInfo*)(peer->data))->rawName == "esc" || ((PlayerInfo*)(peer->data))->rawName == "random")
							{
						...
						{
						if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999) {
							if (((PlayerInfo*)(peer->data))->rawName == "ipadtopia" || ((PlayerInfo*)(peer->data))->rawName == "esc" || ((PlayerInfo*)(peer->data))->rawName == "random")
							{
								string editforeg = str.substr(15, cch.length() - 15 - 1);
						...
						if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999) {
							...
							if (isVIP(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 0) {
									...
									++currentPeer)
								{
									...
									if (((PlayerInfo*)(peer->data))->currentWorld == ((PlayerInfo*)(currentPeer->data))->currentWorld)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Used `2Clear world! No abuse please"));
										ENetPacket * packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(peer, 0, packet);
						...
						if (((PlayerInfo*)(peer->data))->boughtMOD == false)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You need to be `1VIP `oBefore you can do /invis!"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
									...
									++currentPeer)
								{
									...
									if (isHere(peer, currentPeer))
									{
										...
										if (((PlayerInfo*)(peer->data))->boughtVIP == false) {
											...
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`e[MOD LOGS]: `r" + dupa + "`r just doing /invis!"));
											ENetPacket* packet = enet_packet_create(p.data,
												p.len,
												ENET_PACKET_FLAG_RELIABLE);
											enet_peer_send(currentPeer, 0, packet);
									...
									++currentPeer)
								{
									...
									if (isHere(peer, currentPeer))
									{
										...
										if (((PlayerInfo*)(peer->data))->boughtVIP == false) {
											...
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`e[MOD LOGS]: `r" + dupa + "`r just doing /vis!"));
											ENetPacket* packet = enet_packet_create(p.data,
												p.len,
												ENET_PACKET_FLAG_RELIABLE);
											enet_peer_send(currentPeer, 0, packet);
					...
					if (!((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						if (!((PlayerInfo*)(event.peer->data))->haveGrowId)
						{
							...
							if (logStatus == -3) {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Sorry, this account (`5" + ((PlayerInfo*)(event.peer->data))->rawName + "`4) has been suspended.if you have question,dm Coldtopia7567"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
								...
								GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`2Connecting..`w[`1Version: `32.995 `e(c) -`eColdTopia + Anybody`w]"));
								ENetPacket* packet2 = enet_packet_create(p2.data,
									p2.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet2);
								...
								if (((PlayerInfo*)(peer->data))->rawName == "coldtopia") {
								...
								if (((PlayerInfo*)(peer->data))->rawName == "massy") {
									((PlayerInfo*)(event.peer->data))->displayName = "`#" + ((PlayerInfo*)(event.peer->data))->tankIDName;
									((PlayerInfo*)(event.peer->data))->displayNamebackup = "`#" + ((PlayerInfo*)(event.peer->data))->tankIDName;
								}
								if (((PlayerInfo*)(peer->data))->rawName == "focused") {
					...
					if (pStr.substr(0, 17) == "action|enter_game" && !((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						string name = ((PlayerInfo*)(peer->data))->displayName;
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Server Hosted and made by `eColdTopia `1[`4ColdTopia#7567`1]"));
						ENetPacket * packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
						...
						//enet_host_flush(server);
						delete p.data;
						GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`1Welcome Back, " + name + ". `1(`wThere is `2" + std::to_string(counts) + " `wPlayers online!`1)"));
						ENetPacket* packet2 = enet_packet_create(p2.data,
							p2.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet2);
...

						{
							...
							//enet_host_flush(server);
							delete p.data;
							GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`wServer Made by `eColdtopia `1[`4ColdTopia#7567`1]"));
							ENetPacket* packet2 = enet_packet_create(p2.data,
								p2.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet2);
						...
						if (id == "name" && isJoinReq)
						{
							...
							try {
								...
								else
								{
									...
									if (existsban)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Oh no! `oYou've been banned from that world by its owner! Try again later after the worldban wears off``"));
										ENetPacket * packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(peer, 0, packet);
									...
									if (((PlayerInfo*)(peer->data))->isCursed == true)
									{
										...
										if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 666) {
											...
											delete p2x.data;
											GamePacket p = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), ((PlayerInfo*)(peer->data))->netID), "`5<`w" + ((PlayerInfo*)(peer->data))->displayName + "`` `5entered, `w" + std::to_string(otherpeoples) + "`` others here>``"), 0));
											ENetPacket* packet = enet_packet_create(p.data,
												p.len,
												ENET_PACKET_FLAG_RELIABLE);
											enet_peer_send(peer, 0, packet);
										...
										if (((PlayerInfo*)(peer->data))->haveGrowId == false)
										{
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`0Now create your GttcID!"));
											ENetPacket * packet = enet_packet_create(p.data,
												p.len,
												ENET_PACKET_FLAG_RELIABLE);
											enet_peer_send(peer, 0, packet);
							...
							if (data2->packetType == 10)
							{
								...
								case 5:
									...
									if (((PlayerInfo*)(event.peer->data))->cloth5 == pMov->plantingTree)
									{
										...
										((PlayerInfo*)(peer->data))->effect = -1002;
										sendState(peer); //here
										sendPuncheffectpeer(peer, ((PlayerInfo*)(peer->data))->effect);
									}
								...
								case 6:
									...
									else if (pMov->plantingTree == 5078) {
									...
									else if (pMov->plantingTree == 5080) {
										if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
										{
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
											ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5082) {
										if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
										{
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
											ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5084) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5126) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5128) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5130) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5132) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5146) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5144) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5148) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5150) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5162) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5164) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5166) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5168) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5180) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5182) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5184) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 5186) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 7166) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 7168) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 7170) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 7172) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
									...
									else if (pMov->plantingTree == 7174) {
									if (((PlayerInfo*)(event.peer->data))->boughtAAC == false)
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This item must be purchased! type /store to buy it."));
										ENetPacket* packet = enet_packet_create(p.data,
